...
ip addr add 192.0.2.4/32 dev dummy0
ip link set up dev dummy0

ip addr add 192.0.2.13/31 dev eth1
ip link set up dev eth1

ip link add vx0 type vxlan local 192.0.2.4 dstport 4789 external vnifilter nolearning
...
ip link set br0 addr aa:bb:cc:00:00:04
ip link set br0 up
ip link set vx0 up
...
# Spoke-B LAN (eth2) in spoke
ip link set eth2 master spoke
ip addr add 198.51.100.17/28 dev eth2
